...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	...
	LDR R1, =SSI0_SR_R
	LDR R1, [R1]					; R1 is SSIO_SR_R
	AND R1, #0x10					; just bit 4
	CMP R1, #0x10					
	BEQ writecommand				; if bit 4 is high, loop back to beginning of subroutine
	
	LDR R1, =DC
	LDR R2, =DC_COMMAND
	STR R2, [R1]					; clears DC_COMMAND 
	
	LDR R1, =SSI0_DR_R
	STR R0, [R1]					; stores command into SSIO_DR_R
...
loop
	...
	AND R1, #0x10					; just bit 4
	CMP R1, #0x10					
	BEQ loop						; if bit 4 is high, loop back to lp
...
;4) Write the 8-bit data to SSI0_DR_R
	...
	LDR R1, =SSI0_SR_R
	LDR R1, [R1]					; R1 is SSIO_SR_R
	AND R1, #0x02					; just bit 1
	CMP R1, #0x00					
	BEQ writedata					; if bit 4 is low, loop back to beginning of subroutine
	
	LDR R1, =DC
	LDR R2, =DC_DATA
	STR R2, [R1]					; sets DC_DATA					
	
	LDR R1, =SSI0_DR_R
	STR R0, [R1]					; stores data into SSIO_DR_R
