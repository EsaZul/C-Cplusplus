...
void PortF_Init(void){
	...
	GPIO_PORTF_DIR_R |= 0x0E;									// sets PF 3,2,1 as outputs
	GPIO_PORTF_DEN_R |= 0x0E;									// digital enable PF 3,2,1
...
uint32_t Convert(uint32_t input){
	int position = 2100*input/(4095+0.2532);
  return position;											// for distace of 1.5cm
}
...
//************************ MAIN Program?? ************************
int main(void){													// CHANGE realmain TO main bc only way it works
  TExaS_Init();
//************************ LAB 8 ************************
	ST7735_InitR(INITR_REDTAB); 
  PortF_Init();
