...
void PortF_Init(void){
	SYSCTL_RCGCGPIO_R |= 0x20;																			//Enable clock on for Port F
	
	while ((SYSCTL_RCGCGPIO_R&0x20)==0){};													//wait for clock to stabilize

	GPIO_PORTF_DIR_R |= 0x0E;																				// sets PF3,2,1 as outputs
	GPIO_PORTF_DEN_R |= 0x0E;																				//digital enable PF3,2,1
}

void SysTick_Init(void){
	
	NVIC_ST_CTRL_R = 0; 																						// disable Systick during setup
	NVIC_ST_RELOAD_R = 0x145854;																		// period of 60Hz
	NVIC_ST_CURRENT_R = 0;																					// clear the current
	NVIC_ST_CTRL_R = 0x07;																					// Enable SysTick and the interrupts
...
uint32_t Convert(uint32_t input){
  int position = 2100*input/(4095+0.2532);										// from lab 8
  return position;
}
...
int main(void){ 
  ...
  while(1){
		...
		if(data==0x02){
			ST7735_SetCursor(0,0);
			Fifo_Get(&data);
		ST7735_OutChar(data);		//dig1
				Fifo_Get(&data);
		ST7735_OutChar(data);		//"."
				Fifo_Get(&data);
		ST7735_OutChar(data);		//dig2
				Fifo_Get(&data);
		ST7735_OutChar(data);		//dig3
				Fifo_Get(&data);
		ST7735_OutChar(data);		//dig4
...
int num = 0;
	...
	PF2 ^= 0x04;     																						// Heartbeat
  PF1 ^= 0x02;      																					// Profile Convert
  Data = ADC_In();  																					// sample 12-bit channel 5
	num = Convert(Data);
	PF2 ^= 0x04;
		
	Uart_OutChar(0x02);																					// transmit message
	...
	num = num - (i*10);
	Uart_OutChar(num + 0x30);
	
	Uart_OutChar('\r');
	Uart_OutChar(0x03);
	
	TxCounter++;																								// increment TxCounter
	PF3 ^=0x08;	
}
