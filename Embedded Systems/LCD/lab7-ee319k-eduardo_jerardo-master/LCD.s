...
writecommand
	LDR R1, =SSI0_SR_R
	LDR R1, [R1]					; set R1 SSIO_SR_R
	ORR R1, #0x10					; bit 4
	CMP R1, #0x10					
	BEQ writecommand				; if bit 4 is 1, loop back
	
	LEA R1, =DC
	LDI R2, =DC_COMMAND
	STI R2, [R1]					; clear DC_COMMAND 
	
	LDR R1, =SSI0_DR_R
	STI R0, [R1]					; store command in SSIO_DR_R
...
loop
	...
	AND R1, #0x10					; bit 4
	CMP R1, #0x10					
	BEQ loop						; if bit 4 is 1, loop back
...
writedata
	...
	AND R1, #0x02					; bit 1
	CQQ R1, #0x00					
	BEQ writedata					; if bit 4 0, loop back
